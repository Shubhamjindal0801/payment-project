...
const FriendsMenu = (props: Props) => {
  ...
  const getFriendList = (time: number) => {
    setIsLoading(true);
    setTimeout(async () => {
      await axios
  ...
  const handleAddFriend = async () => {
    await axios
      .post(`http://localhost:8080${apiContract.addFriend}/${creatorId}`, {
  ...
  const handleRemoveFriend = async (friendId: any) => {
    console.log("shubham2", friendId);
    await axios
      .post(`http://localhost:8080${apiContract.removeFriend}/${creatorId}`, {
        friendId: friendId,
      })
      .then((res) => {
        message.success(res.data.message);
